...
\section{Step Size}
For the unconstrained case, once a search direction is chosen an optimal step size can be computed with the cost of two mat-vec's: 
\begin{align*}
    \eta^* &= \argmin_\eta \langle \x + \eta\q, \tfrac{1}{2}\A(\x + \eta\q) +\b\rangle \\
    0 &\stackrel{set}{=} \frac{d}{d\eta } \bigl(\langle \x + \eta\q, \tfrac{1}{2}\A(\x + \eta\q) +\b\rangle\bigr)\\
    \eta^* = h(\q) &=: -\frac{\tfrac{1}{2} (\langle \x, \A \q\rangle + \langle \q, \A \x\rangle) + \langle \q, \b\rangle}{\langle \q, \A \q\rangle}
\end{align*}

In the constrained case, the following iteration could be used 
\begin{align*}
    \p_k &= \H_k \nabla f_k &\text{(Quasi-Newton Step Direction)}\\
    \kappa_k &= 1 &\text{(Unconstrained Step Size)}\\
    \bar{\x}_k &= \prox_{\x\geq 0}^{\B_k}(\x_k +\kappa_k \p_k) &\text{(Apply Proximal Operator)}\\
    \q_k &= \bar{\x}_k - \x_k &\text{(Feasible Step Direction)}\\
    \eta^* &= \frac{\langle \nabla f_k, \q_k\rangle }{\langle \q_k,\A\q_k\rangle} &(\text{Unconstrained Optimal Step Size}) \\
    \hat{\boldsymbol{\eta}} &= - \frac{\x[\q<0]}{\q[\q<0]} &(\text{Possible Break Points}) \\
    \eta_k &= \min(\eta^*, \hat{\boldsymbol{\eta}}) &(\text{Constrained Optimal  Step Size}) \\
    \x_{k+1} &= \x_k + \eta_k \q_k &\text{(Next Iterate)}
\end{align*}

In the literature it has been shown for quasi-Newton methods (without limited memory) that $\kappa^* \rightarrow 1$. In the limited memory and constrained case, there is not clear theory on the optimal step size here, but experimentally a choice of 1 is competitive.

Using the optimal step size $\eta$ after the proximal step does not require the use of an additional mat-vec because we can get the next gradient computation for free by caching $\A\q_k$. Experimentally, this gives competitive results.
\[\nabla f_{k+1} = \A\x_k + \eta_k \A\q_k + b\] 
...
\begin{table}[ht]
    ...
    \caption{Table for $n\in[100, 150]$}
\end{table}
\subsection{Random Problems Problems}
\paragraph{No Structure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{randProblems_noStructure_updateStepSize_n_100_150_boxPlot.pdf}
    \caption{\protect \input{fig/randProblems_noStructure_updateStepSize_n_100_150_caption.tex}}
    \label{fig:ammphi-med}
\end{figure} 

\begin{table}[ht]
    \resizebox{\textwidth}{!}{
    \input{fig/randProblems_noStructure_updateStepSize_n_100_150_table.tex}
    }
    \caption{Table for $n\in[150, 200]$}
\end{table}
\pagebreak
