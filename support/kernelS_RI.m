function [GS,GRI] = kernelS_RI(S, GRI, directIn)
...
%pDirect = 25; %empirical form timeKernelS()
...
%if( direct && ( any([S.stokesOpStale]) || any(S ~= lastSurf) ) ) %%length(S) == 1 via direct
    lastSurf = S;
    [GS,GRI] = stokesKernelMat(to_array(S.cart), S.geoProp.W, S.p,GRI);
    S.stokesOpStale = false;
...
function [GS,GRI] = stokesKernelMat(X, W, p,GRI)
