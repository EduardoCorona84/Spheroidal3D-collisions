function Y = Sh_Kernel_Eval_off(phi,type,doAna,out,rho,u,v,Nr)
%{
...
% Synthesize
if d2>1 || ~isempty(u)
    Y = LOCAL_off_shSyn(p,Sqh,out,type,rho,u,v,Nr); 
else
    Y = LOCAL_self_shSyn(p,Sqh,out,type,rho); 
end
   ...
   % Build dense matrix
   Y = LOCAL_shSyn_matrix(p,d2,out,type,rho,u,v,Nr,Rdiag); 
end

end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Yq = LOCAL_self_shSyn(p,qh,out,type,r)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Yq = LOCAL_off_shSyn(p,qh,out,type,r,u,v,Nr)
...
Fr = fun(repmat(r,1,p+1),repmat(0:p,ntrg*d2,1)); 
...
Sfun = LOCAL_get_Frn([type(1) 'Mat'],out); 
SFr = Sfun(repmat(r,1,p+1),repmat(0:p,ntrg*d2,1))./repmat(r+(r==0),1,p+1); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function K = LOCAL_shSyn_matrix(p,d2,out,type,r,u,v,Nr,Rdiag)
...
T = shAna(eye(np)); %Analysis operator
...
DYext = @(r,n) n.*r.^(-n-1);
DYint = @(r,n) -(n+1).*r.^n; 
DpYext= @(r,n) -n.*(n+1).*r.^(-n-2);
DpYint= @(r,n) -n.*(n+1).*r.^(n-1);
if out
%Outside sphere
    switch type 
        case 'SMat'
...
ShKq = Sh_Kernel_Eval_off(qh,'SMat',0,out,rho,u,v);
ShdKq = Sh_Kernel_Eval_off(qh,'SpMat',0,out,rho,u,v);
ShdKq2 = Sh_Kernel_Eval_off(qh,'SpMat',0,out,rho,u,v,dpar.nor);
...
dKq = Kernel_Eval(Xtrg,X,dpar)*q;
ShdKq = Sh_Kernel_Eval_off(qh,'SpMat',0,out,rho,u,v,dpar.nor);
...
ShKq = Sh_Kernel_Eval_off(q,'SMat',1,out,rho,u,v);
ShdKq = Sh_Kernel_Eval_off(q,'SpMat',1,out,rho,u,v);
ShdKq2 = Sh_Kernel_Eval_off(q,'SpMat',1,out,rho,u,v,dpar.nor);
...
dpar.nor = Nr; 
dKq = Kernel_Eval(Xtrg,X,dpar)*q;
ShdKq = Sh_Kernel_Eval_off(q,'SpMat',1,out,rho,u,v,Nr);
fprintf('rel error for Sp outside (Nr): %e\n',norm(dKq-ShdKq)/nrmq)
