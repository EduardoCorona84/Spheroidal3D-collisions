function [Y,Itrg,neigh] = VSh_Mod_MatVec_RB_trg(V,Xtrg,Nrtrg,params)
%{
...
if ~isempty(V) && isnumeric(V)  
    ...
    for nbox=1:n3
        ...
        if kerd==1
            Ynear = Sh_Mod_Kernel_Eval_off(Vh_ngh,pMat,0,out,rho,phi,th,Nrtrgb,lambdar);
        elseif kerd==3
            Ynear = Vsh_Kernel_Eval_off(Vh_ngh,pMat,0,out,rho,phi,th,Nrtrgb,lambdar);
        end
...
elseif strcmp(V,'Mat')
    ...
    for nbox=1:n3
        ...
        if kerd==1 
            Ynear=Sh_Mod_Kernel_Eval_off([p 1],pMat,0,out,rho,phi,th,Nrtrgb,lambdar);
        elseif kerd==3
    ...
    Y = @(V) VSh_Mod_MatVec_RB_trg(V,L,params);
end
