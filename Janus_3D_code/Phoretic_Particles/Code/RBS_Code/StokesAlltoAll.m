function pot = StokesAlltoAll(src, den, trg)
% SRC and DEN are ordered per vesicle, TRG is a single column vector assumed to
% be in the [x;y;z] format.

  if(nargin==0), testThis(); return; end
  dot3 = @(x,y) sum(reshape(x,[],3) .* reshape(y,[],3),2);
  
  srcIsTrg = false;
  if(nargin<3 || isempty(trg)) 
    nv = size(src,2);
    trg = src'; 
    srcIsTrg = true;
  end
  ...
  for ii=1:ntrg
    ...
    rf = dot3(r, den);
    rf = rf .* invRho .* invRho;
    pp = den + repmat(rf,1,3).* r;
    pp = pp .* repmat(invRho,1,3);
    pot(ii,:) = sum(pp,1);            %make sure is correct
  end
...
function testThis()
  ...
  den(:,2) = ones(nsrc,1)/nsrc;
  src = src(:);
  den = den(:);
  trg = [2 0 0]';
  pot = StokesAlltoAll(src, den, trg);
  pot(2) = pot(2) - log(trg(1));
