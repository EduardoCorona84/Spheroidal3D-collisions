...
if isempty(init)
   ...
   Fparams.parslv.prec=prec; 
   Fparams.parslv.prev=prev; 
...
if ~strcmp(Fparams.parbd.Shape,'') 
   p2=2*Fparams.parbd.p;
   Sc2 = SurfaceSph(shape_gallery(2*Fparams.parbd.p,Fparams.parbd.Shape)); 
   % Model surface pts
...
tic; 
[FT2,sigma2,VW2] = LOCAL_get_incoming_Fc(Fparams,t+dt,dt,Kernels,Nullsp,Xt1,Sc); 
...
% Get incoming force distribution: 
tic; 
[FT42,sigma42,VW42] = LOCAL_get_incoming_Fc(Fpar41,t42,dt42,Ker41,Null41,Xt41,Sc); 
fprintf('\n Time to compute incoming force: %e',toc);
timings.incoming(i) = timings.incoming(i)+toc;  
fprintf('\n Fluid Solve at time %.2f',t)
tic; 
...
fprintf('\n dt: %2.2f ',dt)
if mod(i,2)==1                                                                                                                                              
save(fname,'-v7.3','tt','Xt','Mt','Ct','FT','sigma','mu','U','VW');                                                                                         
else                                                                                                                                                        
save([fname '2'],'-v7.3','tt','Xt','Mt','Ct','FT','sigma','mu','U','VW');                                                                                   
end  
...
function [Xtp,Mtp,Ctp,U,FT,sigma,mu,VW,Kernels,Nullsp,Fparams,colevent,collist,dt] = LOCAL_euler_step(Xt,X0,X2,Mt,Ct,Kernels,Nullsp,Fparams,colevent,collist,t,dt,it)
global timings; 
Sc = Fparams.parbd.Sc; 
...
% Get incoming force distribution: 
tic; 
[FT,sigma,VW] = LOCAL_get_incoming_Fc(Fparams,t,dt,Kernels,Nullsp,Xt,Sc); 
fprintf('\n Time to compute incoming force: %e ',toc)
...
function [FT,fM,VW] = LOCAL_get_incoming_Fc(Fparams,t,dt,Kernels,Nullsp,Xt,Sc)
...
ftmp = (1/4/pi)*maxwellSnor(H_e{j},Sc{p,sj}) - maxwellSnor(H_i{j},Sc{p,sj}); 
ftmp = real(reshape(ftmp.to_array,[],3))'; 
fM(indv) = ftmp(:); 
end

if isfield(Fparams,'Ffun')
   Ffun = Fparams.Ffun; 
   Tfun = Fparams.Tfun; 
   Ct = Fparams.parbd.C; 
   ...
   display(reshape(real(Ck*fM),6,n3))
   display(reshape(FTex,6,n3))
...
FT2 = reshape(FT,6,n3);
display(FT2)
display(sqrt(FT2(1,:).^2 + FT2(2,:).^2 + FT2(3,:).^2))

elseif strcmp(Fparams.type,'3Sphx')
...
elseif strcmp(Fparams.type,'3SphGolx')
TD = Kernels.TD; SD = Kernels.SD;     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmp(Fparams.type,'Purcellxy')
TD = Kernels.TD; SD = Kernels.SD;     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmp(Fparams.type,'FTfun') || nargin<4
   Ffun = Fparams.Ffun; 
   Tfun = Fparams.Tfun; 
   Ct = Fparams.parbd.C; 
...
if isnumeric(A) && ~strcmp(parslv.solver,'gmres')
    x=A\b; 
else
    x = zeros(size(b)); 
    %b = parslv.Pr(b); 
...
if max(abs(Uinf))>1e-3
    fprintf('\n WARNING: flow at boundary too large'); pause; 
...
%{
if ~strcmp(Shape,'') && ~isempty(collist)
    np2 = size(X2,1)/n3;
