function [Y,params] = Periodic_SelfEval(p,C,r,a,b,flag_pot,dense,neigh,Q)
%{
...
Y = S_{Gamma_p}[Q](x) = int_{Gamma_p} K(x,y)Q(y)dSy    ;  x in Gamma_p
...
neigh - (bool) option to add neighboring copies
...
params = LOCAL_setparams(p,C,r,a,b,flag_pot,dense,neigh); 

L = []; %Can change this to block-diagonal correction for traction BIE / to 
% completion flow for Power-Miranda, etc. 

if isempty(Q)
    Y = VSh_MatVec_RB2('Mat',L,params); 
else
    % Eval routine (w/ spherical harmonics for near and self-eval)% Spherical harmonics eval
    Y = VSh_MatVec_RB2(Q,L,params);  
end


end

function params = LOCAL_setparams(p,C,r,a,b,flag_pot,dense,neigh)
...
if kerd>1
params.ci = repmat((1:kerd)',size(Xg,1),1); params.cj = params.ci; 
end
