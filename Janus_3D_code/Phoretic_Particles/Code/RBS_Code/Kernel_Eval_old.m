...
elseif params.dim == 2
 ...
 % 3D (surfaces and volume)   
     ...
     case 'SL_L_3D'  
        ...
        A = a*(den==0) + wh.*b.*(1./sqrt(den + (den==0)) - (den==0)).*c; 
     case 'DL_L_3D'  
     ...
     % RPY Matrix 3D (assumes a=1 for now) 
     case 'RPY_Stk_3D'
     ...
     %Build rxr first 
     RXR = ones(size(den)); 
     RXR(I1) = RXR(I1).*d1(I1); RXR(J1) = RXR(J1).*d1(J1);
     RXR(I2) = RXR(I2).*d2(I2); RXR(J2) = RXR(J2).*d2(J2);
     RXR(I3) = RXR(I3).*d3(I3); RXR(J3) = RXR(J3).*d3(J3);
     
     A = zeros(size(den));  
     
     % Anear = (k*T/6*pi*mu*a)[(1-(9/32)*(r/a)) I + (3/32a)(rxr/r)]
     A(nr) = IIDX(nr).*(a*(den(nr)==0) + (1/6)*wh(nr).*(1-(9/32)*den(nr))) ...
         + (1/64)*wh(nr).*RXR(nr)./((den(nr)+(den(nr)==0)));
     
     % Afar = (k*T/8*pi*mu)[(1/r + 2a^2/3r^3) I + (rxr/r.^3 - 2a^2 rxr/r^5)]
     A(fr) = (1/8)*wh(fr).*(IIDX(fr).*(1./den(fr) + (2/3)./(den(fr).^3)) ...
         + RXR(fr).*(1./(den(fr).^3) - 2./(den(fr).^5)));      
