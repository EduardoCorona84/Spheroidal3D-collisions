function  DfAll = kerneldSMatFree(f, S)
...
%
  ...
  for vesIdx=1:nv
    ...
    for ii=1:floor(p/2)+1
      ...
      nrp = nr(northHem(ii,:),:);
      nrp = reshape(nrp,1,[]); 
      nrp = repmat(nrp,p+1,1); 
      nrp = reshape(nrp,3*np,2*p);
      %The distance and stokes kernel
      rho = sqrt(dot3(r,r)); 
      rho = repmat(rho,3,1);
      
      rho = Wr.*((repmat(dot3(r,fr).*dot3(r,nrp),3,1)).*r./rho.^5);% + ...
        % (repmat(dot3(r,nrp),3,1).*fr - ...
        %  repmat(dot3(r,fr),3,1).*nrp - ...
        %  repmat(dot3(nrp,fr),3,1).*r)./rho.^3);
      %dumping the results
