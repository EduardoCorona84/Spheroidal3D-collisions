function  SfAll = kernelSMatFree(f, S)
% KERNELSMATFREE - calculates the stokes kernel matvec of the surface S,
% with density f.
% 
% EXAMPLE: 
%     p = 12;
%     S = SurfaceSph(shape_gallery(p,''));
%     f = vec3d( rand(2*p*(p+1),3) );
%     Sf = kernelSMatFree(f,S);
%
...
%
  ...
  if( nargin == 0 ), testSMF(); return; end
  ...
  if(isempty(p) || p~=S(1).p)
    ...
    printMsg('* Generating the fast Stokes data for p=%d.\n',p);
    [u, v] = gl_grid(p);
    ...
    qw = (1/8/pi)*SingularWeights(p); qw = qw(:); 
    qwIm = flipud(reshape(qw,p+1,[])); qwIm = qwIm(:);
  end
  
  %- treating vesicles one by one, this can be optimised.
  fAll = f; 
  ...
  for vesIdx=1:nv
    ...
    for ii=1:floor(p/2)+1
      ...
      rho = sqrt(dot3(r,r)); 
      rho = repmat(rho,3,1);
      rho = Wr.*(fr./rho + repmat(dot3(r,fr),3,1).*r./rho.^3);
      %dumping the results
      Sf(northHem(ii,:),1) = sum(rho(     1:  np,:));
      Sf(northHem(ii,:),2) = sum(rho(  np+1:2*np,:));
      Sf(northHem(ii,:),3) = sum(rho(2*np+1:3*np,:));
      ...
      if(ii~=p/2+1)
        ...
        rho = Wr.*(fr./rho + repmat(dot3(r,fr),3,1).*r./rho.^3);
        %dumping the results
        Sf(southHem(ii,:),1) = sum(rho(     1:  np,:));
        Sf(southHem(ii,:),2) = sum(rho(  np+1:2*np,:));
        Sf(southHem(ii,:),3) = sum(rho(2*np+1:3*np,:));
      end
    end
    Sf = Sf(:);
    SfAll(vesIdx)  = vec3d( Sf );
  end
...
function testSMF()
  ...
  F1 = S.stokesMatVec( f, opts);
  F2 = kernelSMatFree( f, S);
