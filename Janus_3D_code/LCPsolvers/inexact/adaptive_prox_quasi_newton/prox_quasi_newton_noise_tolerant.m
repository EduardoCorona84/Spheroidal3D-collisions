function [x, iter, errStruct] = prox_quasi_newton_noise_tolerant(fcnGrad, x0, opts)
...
for k = 1:opts.max_iter
    ...
    if opts.noise_control_quadratic == true
        q = x_k - x_km1;
        Qqvec = opts.Q*q;
        eta = min(-dot(q, grad_k)/ dot(q, Qqvec),1);
        x_k = x_km1 + eta*q;
...
function [sk, yk] = prox_search(x0, grad0, kappa0, h0, U, V, fcnGrad, opts)
    ...
    while noisey
        ...
        if ((grad1 - grad0)'*(x1 - x0)) > 2*(1 + opts.noise_control_parameter)*opts.noise 
            noisey = false;
        end
    end
