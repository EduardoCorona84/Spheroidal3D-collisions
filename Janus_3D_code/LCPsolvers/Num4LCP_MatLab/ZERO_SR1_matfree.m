function [ x, err,  iter, flag, convergence, msg] = ZERO_SR1_matfree( A, b, x0, max_iter, tol_rel, tol_abs, profile )
...
fcnGrad = @(x) quadprog_matfree(x,A,b);
opts = struct( ...
    'N',size(A,2), ...
    'verbose',25, ...
    'nmax',max_iter, ...
...
function [f,g] = quadprog_matfree(x, A, b)
    Ax = A(x);
    f = 1/2*dot(Ax,x) + dot(b,x);
    if nargout < 2 
        return 
    end
    g = Ax + b;
