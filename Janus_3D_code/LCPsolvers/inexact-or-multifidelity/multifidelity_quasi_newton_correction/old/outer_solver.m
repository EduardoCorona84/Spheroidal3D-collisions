function [x_inner_0, opts] = outer_solver(x_outer_1, grad_outer_1, s_1, y_1, outer_iter, opts)
    ...
    switch lower(opts.outer.solver)
        ...
        case 'prox'
            ...
            %Add support for optimal step length here. If we are taking a low fidelity step afterwards, we can't reuse this gradient evaluation. If we aren't we can. 
        case 'qn'
            % unconstrained, normal, quasi-Newton
            %we can still update everything as we would with prox, just does extra work. 
            opts.outer.qn = updateH(outer_iter, s_1, y_1, opts.outer.qn);

            % Take the quasi-Newton step
            p = -opts.outer.qn.H(grad_outer_1);
            x_inner_0 = x_outer_1 + p;
    end
...
function opts = updateH(outer_iter, s_1, y_1, opts)
    ...
    if ~isempty(skipped_r) && r < outer_iter - 1
        r = max(skipped_r-1, 1);
    end
