function data = fixed_update_fixed_memory_multi_inner(A, A_low, b, parameters, opts, x_ref)
    ...
    %3. parameters.inner_iters, the number of inner iterations to be used, this is a vector
    ...
    for i = 1:length(parameters.inner_iters)
        inner_iter = parameters.inner_iters(i);
        opts.inner.enabled = true;
        opts.inner.solver_opts.max_iter = inner_iter;
        opts.outer.correction = true;
        opts.outer.storeIters = true;
        opts.outer.correction_opts.r = parameters.memory.r;
        opts.outer.correction_opts.update = parameters.update;
        ...
        data.(['inner_' num2str(inner_iter)]).abs_error = abs_error;
        data.(['inner_' num2str(inner_iter)]).rel_error = rel_error;
    end
end
