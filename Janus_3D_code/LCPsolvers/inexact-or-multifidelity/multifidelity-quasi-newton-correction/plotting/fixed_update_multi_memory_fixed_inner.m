function data = fixed_update_multi_memory_fixed_inner(A, A_low, b, parameters, opts, x_ref)
    ...
    for i = 1:length(parameters.inner_iters)
        memory.r = parameters.memory.r(i);
        memory.type = parameters.memory.type(i);
        ...
        opts.outer.correction_opts.r = memory.r;
        opts.outer.correction_opts.update = parameters.update;
        opts.outer.correction_opts.memory = memory.type;
        ...
        data.(['memory_' num2str(memory.r)]).abs_error = abs_error;
        data.(['memory_' num2str(memory.r)]).rel_error = rel_error;
    end
end