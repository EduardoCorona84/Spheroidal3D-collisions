function opts = get_H_BFGS_multi(k, s, y, opts, bMask, debug)
    ...
    for i = 1:r
        ...
        if i > 1 
            ...
            v = v + opts.U(~bMask,1:i-1)*ucoeff - opts.V(~bMask,1:i-1)*vcoeff;
        end
        opts.V(~bMask,i) = v / sqrt(dot(s,v));
    end
    ...
    if debug 
        ...
        Htest = eye(n);
        for i = 1:n 
            Htest(:, i) = opts.H(Htest(:,i));
        end
        Btest = 1/h0*eye(n) + opts.U*opts.U' - opts.V*opts.V';
        assert(norm(Htest(~bMask, ~bMask) - Hexp) / norm(Hexp) < 1e-6);
        assert(norm(Btest(~bMask, ~bMask)  - Bexp) / norm(Bexp) < 1e-6);
        assert(norm(opts.U(~bMask,:) - Uexp) / norm(Uexp) < 1e-6);
        assert(norm(opts.V(~bMask,:) - Vexp) / norm(Vexp) < 1e-6);
    end 
end % get_H_BFGS
