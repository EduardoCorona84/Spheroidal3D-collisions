...
for p = 4:7
    for tol = [1e-5] %, 1e-6, 1e-7, 1e-8]
        ...
        algoHndls{end+1} = @(fg, x0, opts) precProxQuasiNewton(fg, x0, opts,...
            (res_.A{opts.i,jMid,kMid} + res_.A{opts.i,jMid,kMid}') /2, res_.b{opts.i}, ...
        ...
        % Use the BFGS to get a low rank approx of Ahat but don't use it
        % algoNames{end+1} = ['PQN (BFGS) $x_0 \leftarrow \hat{A}(' sprintf('p=%d', p) ...
        %     ', \epsilon_{\mathrm{gmres}}=' sprintf('%.1g', tol) '$'];
        % algoSlvr{end+1} = 'proxquasinewton';
        % algoHndls{end+1} = @(fg, x0, opts) precProxQuasiNewton(fg, x0, opts,...
        %     (res_.A{opts.i,jMid,kMid} + res_.A{opts.i,jMid,kMid}') /2, res_.b{opts.i}, ...
        %     'solveLoFiThenThrowAway',p,tol,mRatio);
    end
end
%%
