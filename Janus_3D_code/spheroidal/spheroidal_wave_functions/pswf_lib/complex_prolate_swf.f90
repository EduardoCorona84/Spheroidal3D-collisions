...
!  scalars
        real(knd) aj1, aj2, ang, apcoef, apcoefn, c, coefme, coefmo, &
                  ...
                  xbninp, x1
        real(knd), parameter :: x1_eta_min = 0.4e0_knd
        real(knd), parameter :: x1_leg_min = 0.4e0_knd
...
!  arrays with dimension lnum
        integer, allocatable :: iqdl(:), iql(:), ifajo(:), ieig(:)
        real(knd), allocatable :: qdl(:), ql(:)
        complex(knd), allocatable :: fajo(:), eig(:), eigst(:)
...
!  arrays with dimension maxd
        complex(knd), allocatable :: enr(:), bliste(:), gliste(:), blisto(:), &
                                     glisto(:)
        complex(knd1), allocatable :: bliste1(:), blisto1(:), gliste1(:), &
                                      glisto1(:)
!
!  arrays with dimension maxdr
        complex(knd), allocatable :: drhor(:)
!
!  arrays with dimension maxint
        complex(knd), allocatable :: pint1(:), pint2(:), pint3(:), &
                                     pint4(:), rpint1(:), rpint2(:)
!
!  arrays with dimension maxj
        complex(knd), allocatable :: sbesf(:), sbesf2(:), sbesdf(:), &
                                     sbesdf2(:)
!
!  arrays with dimension maxlp
        integer, allocatable :: ibese(:), ibese2(:), ipnormint(:)
        real(knd), allocatable :: pnormint(:)
        complex(knd), allocatable :: sbesdr(:), sbesdr2(:), sbesn(:), &
                                     sbesn2(:)
        complex(knd), allocatable :: sneudre(:), sneudrsv(:,:)
        complex(knd), allocatable :: sneudr(:)
!
!  arrays with dimension maxmp
        complex(knd), allocatable :: enrneg(:)
!
!  arrays with dimension maxn
        integer, allocatable :: ineue(:), ineuee(:), ineuesv(:,:)
        complex(knd), allocatable :: sneufe(:), sneudfe(:), &
                                     sneufsv(:,:), sneudfsv(:,:), &
                                     sneune(:), sneunsv(:,:)
        complex(knd), allocatable :: sneun(:), sneuf(:), sneudf(:)
!
!  arrays with dimension given by maxp
        real(knd), allocatable :: alpha(:), beta(:), coefa(:), coefb(:), &
                                  coefc(:), coefd(:), coefe(:), gamma(:), &
                                  pdr(:,:), pdrat(:,:), pdratt(:), &
                                  pr(:,:), prat(:,:), pratb(:), pratt(:), &
                                  prat1(:), pratbsv(:,:), &
                                  prattsv(:,:), pdratsv(:,:)
...
!  arrays with dimension maxpdr
        real(knd), allocatable :: prx(:), pdrx(:)
!
!  arrays with dimension maxq
        real(knd), allocatable :: qr(:), qdr(:)
!
!  arrays with dimension maxt
        real(knd) barg(maxt)
        real(knd), allocatable :: etainp(:), pdnorm(:), &
                                  pdnorma(:), pnorm(:), pnorma(:), pdtempe(:), &
                                  pdtempo(:), ptempe(:), ptempo(:), xin(:), &
                                  xlninp(:)
        complex(knd), allocatable :: s1c(:), s1dc(:)
        integer, allocatable :: ipdnorm(:), ipdnorma(:), ipnorm(:), &
                                ipnorma(:), ipdtempe(:), ipdtempo(:), &
                                iptempe(:), iptempo(:), is1e(:), is1de(:), &
                                naccs(:), naccds(:)
!
!  arrays with dimension neta
        real(knd), allocatable :: eta(:), wmeta2(:), xbn(:), xln(:)
!
!  arrays with dimension ngau
        real(knd), allocatable :: wr(:), xr(:)
!
!  miscellaneous integer arrays
        integer nees(100), naccsav(100)
        integer, allocatable :: neeb(:), limpsv(:), limnsv(:), jelimsv(:)
!
        character chr_w, chr_e
!
!  Allocate work arrays on the heap to avoid stack overflows.
        allocate(iqdl(lnum), iql(lnum), ifajo(lnum), ieig(lnum))
        allocate(qdl(lnum), ql(lnum))
        allocate(fajo(lnum), eig(lnum), eigst(lnum))

        allocate(enr(maxd), bliste(maxd), gliste(maxd), blisto(maxd), glisto(maxd))
        allocate(bliste1(maxd), blisto1(maxd), gliste1(maxd), glisto1(maxd))
        allocate(drhor(maxdr))
        allocate(pint1(maxint), pint2(maxint), pint3(maxint), pint4(maxint), rpint1(maxint), rpint2(maxint))
        allocate(sbesf(maxj), sbesf2(maxj), sbesdf(maxj), sbesdf2(maxj))

        allocate(ibese(maxlp), ibese2(maxlp), ipnormint(maxlp))
        allocate(pnormint(maxlp))
        allocate(sbesdr(maxlp), sbesdr2(maxlp), sbesn(maxlp), sbesn2(maxlp))
        allocate(sneudre(maxlp), sneudrsv(jnenmax, maxlp), sneudr(maxlp))

        allocate(enrneg(maxmp))

        allocate(ineue(maxn), ineuee(maxn), ineuesv(jnenmax, maxn))
        allocate(sneufe(maxn), sneudfe(maxn), sneufsv(jnenmax, maxn), sneudfsv(jnenmax, maxn), &
                 sneune(maxn), sneunsv(jnenmax, maxn))
        allocate(sneun(maxn), sneuf(maxn), sneudf(maxn))

        allocate(alpha(maxp), beta(maxp), coefa(maxp), coefb(maxp), coefc(maxp), coefd(maxp), coefe(maxp), gamma(maxp))
        allocate(pdr(maxt, maxp), pdrat(maxt, maxp), pdratt(maxp), pr(maxt, maxp), prat(maxt, maxp), &
                 pratb(maxp), pratt(maxp), prat1(maxp), pratbsv(jnenmax, maxp), prattsv(jnenmax, maxp), &
                 pdratsv(jnenmax, maxp))

        allocate(prx(maxpdr), pdrx(maxpdr))
        allocate(qr(maxq), qdr(maxq))

        allocate(etainp(maxt), pdnorm(maxt), pdnorma(maxt), pnorm(maxt), pnorma(maxt), pdtempe(maxt), &
                 pdtempo(maxt), ptempe(maxt), ptempo(maxt), xin(maxt), xlninp(maxt))
        allocate(s1c(maxt), s1dc(maxt))
        allocate(ipdnorm(maxt), ipdnorma(maxt), ipnorm(maxt), ipnorma(maxt), ipdtempe(maxt), ipdtempo(maxt), &
                 iptempe(maxt), iptempo(maxt), is1e(maxt), is1de(maxt), naccs(maxt), naccds(maxt))

        allocate(eta(neta), wmeta2(neta), xbn(neta), xln(neta))
        allocate(wr(ngau), xr(ngau))

        allocate(neeb(jnenmax), limpsv(jnenmax), limnsv(jnenmax), jelimsv(jnenmax))
        if (suffix) then
            chr_e = 'e'
            chr_w = 'w'
        else
...
190           continue
              ...
              if(iopint == 3 .and. jint < jintm) jint = jintm
                ...
                if(x1 <= x1_leg_min) then
                  ! Avoid overflow in Wronskian check; return r2int result with low accuracy.
                  naccint = 0
                  naccintp = 0
                  naccout = 0
                  nacccor = 0
...
!  calculation using Legendre expansion and joining factor
              ...
              if(x1 <= x1_leg_min) then
              iopleg = 0
              if(iopneu == 0 .and. iopeta == 0) iopint = 1
              end if
...
700           format(1x, i5, 2x, 2(f17.14, 1x, f17.14, i5, 2x),/,8x, &
              ...
              if(iopeta == 0 .and. x1 > x1_eta_min .and. iopneu == 0 &
...
if (warn) then
 ...
 900         continue
!
!  Release heap-allocated work arrays.
        deallocate(iqdl, iql, ifajo, ieig)
        deallocate(qdl, ql)
        deallocate(fajo, eig, eigst)

        deallocate(enr, bliste, gliste, blisto, glisto)
        deallocate(bliste1, blisto1, gliste1, glisto1)
        deallocate(drhor)
        deallocate(pint1, pint2, pint3, pint4, rpint1, rpint2)
        deallocate(sbesf, sbesf2, sbesdf, sbesdf2)

        deallocate(ibese, ibese2, ipnormint)
        deallocate(pnormint)
        deallocate(sbesdr, sbesdr2, sbesn, sbesn2)
        deallocate(sneudre, sneudrsv, sneudr)

        deallocate(enrneg)

        deallocate(ineue, ineuee, ineuesv)
        deallocate(sneufe, sneudfe, sneufsv, sneudfsv, sneune, sneunsv)
        deallocate(sneun, sneuf, sneudf)

        deallocate(alpha, beta, coefa, coefb, coefc, coefd, coefe, gamma)
        deallocate(pdr, pdrat, pdratt, pr, prat, pratb, pratt, prat1, &
                   pratbsv, prattsv, pdratsv)

        deallocate(prx, pdrx)
        deallocate(qr, qdr)

        deallocate(etainp, pdnorm, pdnorma, pnorm, pnorma, pdtempe, pdtempo, &
                   ptempe, ptempo, xin, xlninp)
        deallocate(s1c, s1dc)
        deallocate(ipdnorm, ipdnorma, ipnorm, ipnorma, ipdtempe, ipdtempo, &
                   iptempe, iptempo, is1e, is1de, naccs, naccds)

        deallocate(eta, wmeta2, xbn, xln)
        deallocate(wr, xr)

        deallocate(neeb, limpsv, limnsv, jelimsv)
        return
          end subroutine
!
!
