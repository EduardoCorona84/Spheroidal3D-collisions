...
params.u0=u0; params.a=a; params.isReal=true; params.oblate=obl;
...
for l=1:length(parr)
    ...
    for j=1:length(distance)
        ...
        % Xseval=Ys; Xseval(:,1)=Xseval(:,1).*(1+distance(j));
        % Xeval=spheroidal2cart(Xseval,a,obl);
        ...
        % soln=spheroidalDL(params,Xeval);
        
        % Actual solution
        truesoln=PtChargePotential(ptch,Xptch,Xeval);
        
        this_p_errs(:,j)=log10(abs(truesoln-soln)./max(abs(truesoln)));
