function modDL = spheroidalModifiedDLP(params, lambda, X_trg, mex_opts)
%--------------------------------------------------------------------%
% spheroidalModifiedDLP computes the modified Laplace double-layer potential.
%    (o) params:
...
function [lambda_int, lambda_surf, lambda_ext] = LOCAL_modDLPspectrum(p, u0, ~, oblate, c, mex_opts)

    if oblate
        % In our oblate modified-Laplace convention, c is real while
        % radial functions are evaluated on cc = 1i*c via sphwv pure-imag.
        % The direct -1i*c prefactor introduces a global +1i phase versus
        % Kernel_Eval(DL_LMOD_3D), so we use -c here.
        cnm = -c * (u0^2 + 1);
    else
        cnm = 1j * c * (u0^2 - 1);
    end
