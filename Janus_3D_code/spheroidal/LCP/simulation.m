function configurations = simulation(initialConfiguration, totalSteps, timeStart, timeEnd)
    ...
    for i = 2:totalSteps + 1
        ...
        [x1Wall, x2Wall, distancesWall, neighborsWall] = allWithWall(configuration, eta);
        ...
        for j = 1:totalParticles
            ...
            disp(forceExternal(6*(j - 1) + 4:6*(j - 1) + 6));
        end
        %}
        ...
        %Now solve the LCP 
        [gamma, ~, ~, ~, ~, ~] = BBPGD(M, q, zeros(size(q)), 2000, 1e-5);
