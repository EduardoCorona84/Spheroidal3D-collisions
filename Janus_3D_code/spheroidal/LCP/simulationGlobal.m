function configurations = simulationGlobal(initialConfiguration, totalSteps, timeStart, timeEnd)
    ...
    for i = 2:totalSteps + 1
        ...
        [x1Wall, x2Wall, distancesWall, neighborsWall] = allWithWallSigned(configuration, [0 0 1].', [0 0 0].', eta);
        ...
        for j = 1:totalParticles
            ...
            for k = 1:length(localNeighbors)
                if localNeighbors(k) == totalParticles + 1
                    ...
                    torque1 = -(cross(localCoordinates1, N));
                    ...
                    torque1 = -(cross(localCoordinates1, N));
                    torque2 = (cross(localCoordinates2, N));
        ...
        for j = 1:totalParticles
            forceExternal(6*(j - 1) + 4:6*(j - 1) + 6) = -cross(velocity(6*(j - 1) + 4:6*(j - 1) + 6),configuration(j).R*diag(((mass/5).*[initialConfiguration(j).b^2 + initialConfiguration(j).c^2 initialConfiguration(j).a^2 + initialConfiguration(j).c^2 initialConfiguration(j).a^2 + initialConfiguration(j).b^2]))*configuration(j).R.'*velocity(6*(j - 1) + 4:6*(j - 1) + 6));
        end
        ...
        %As we are in the global frame, we need to change the mobility matrix at each iteration. Specifically, the moments of inertias need to be rotated to the global frame.
        for j = 1:totalParticles
            mobilityMatrix(6*(j - 1) + 4:6*(j - 1) + 6, 6*(j - 1) + 4:6*(j - 1) + 6) = configuration(j).R*diag(((mass/5).*[initialConfiguration(j).b^2 + initialConfiguration(j).c^2 initialConfiguration(j).a^2 + initialConfiguration(j).c^2 initialConfiguration(j).a^2 + initialConfiguration(j).b^2]).^(-1))*configuration(j).R.';
        end
        ...
        for j = 1:totalParticles
            ...
            G(7*(j - 1) + 4:7*(j - 1) + 7, 6*(j - 1) + 4:6*(j - 1) + 6) = constructPsi(quaternionConfiguration(7*(j - 1) + 4:7*(j - 1) + 7));
        end
...
%This Psi is for a fixed reference frame, not the body frame.
%This suggests all the values we are computing are in the global frame. This means the mobility matrix is probably wrong.
...
function psi = constructLocalPsi(q)
    P = [0 -q(4) q(3);
    q(4) 0 -q(2);
    -q(3) q(2) 0];
