function Y = VSh_Mod_MatVec_RB2(V,L,params)
%{
    ...
    for nbox=1:n3
        ...
        if kerd==1
            if num_ngh>1
                ...
                Ynear(np*(slf-1)+1:np*slf) = Sh_Mod_Kernel_Eval_off(Vh_ngh,pMat,0,out,1,[],[],[],lambdar); 
                Ynear(ind_off) = Sh_Mod_Kernel_Eval_off(Vh_ngh,pMat,0,out,rho(ind_off),phi(ind_off),th(ind_off),Nrtrg(ind_off,:),lambdar);
            else
                Ynear = Sh_Mod_Kernel_Eval_off(Vh_ngh,pMat,0,out,1,[],[],[],lambdar);
            end
        elseif kerd==3
            if num_ngh>1
                slf = find(neigh{nbox}==nbox); 
        ...
        if dense
            ...
            Y(I_nghv,:) = Y(I_nghv,:) + Ynear - Kngh*V(I_box,:); 
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
...
elseif strcmp(V,'Mat')
    ...
    for nbox=1:n3
        ...
        Ynear=Sh_Mod_Kernel_Eval_off([p 1],pMat,0,out,rho,phi,th,Nrtrg,lambdar);
    ...
    Y = @(V) VSh_Mod_MatVec_RB2(V,L,params);
end
