...
polydisperseRatio - (double) maximum purtubation relative to the meanRadiu

Gemoetry
wall - (struct) struct with fields point, normal, eps defining the wall as a plane with a buffer of eps. point is a point on the plane and normal is the normal vector to the plane. 
shell - () TODO
if empty, infinite domain is assumed.
...
%}
function [Fparams]=Test_Mobility_Wall(...
...
if ~exist('wall', 'var') || isempty(wall)
    point = [0 0 -3.5];
    normal = [0 0 1];
end
%% LCP default params
...
fname = fullfile(resDir, ['wall' postFix 'verification.new']); % Name of file for regular results file
LCP_file_path = fullfile(resDir, ['wall.lcp' postFix]); % Name of the LCP results file
...
Fparams = struct('Nt',Nt,'dt',dt,'comp',1,'type','FTfun',...
...
Fparams.parbd.Ct = C;
Fparams.wall = struct('point', point, 'normal', normal, 'eps', ep);
...
Fparams.Ffun = @(t, C) repmat([0; 0; -1.], 1, size(C, 1));
Fparams.Tfun = @(t, C) zeros(3, size(C, 1));
